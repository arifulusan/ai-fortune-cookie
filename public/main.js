...
function wireActions(){
  ...
  shareBtn?.addEventListener('click', async ()=>{
    ...
    if (navigator.share) {
      try{ await navigator.share({title:'Fortuny', text, url:url.toString()}); const old=shareBtn.textContent; shareBtn.textContent=t('shared'); setTimeout(()=>shareBtn.textContent=t('share'),1200);}catch{}
    } else {
...
// Confetti on streak increase (tiny, no libs)
...
async function crackAndReveal(){
  ...
  setTimeout(async ()=>{
    try{ const data=await postFortune(); showReveal(data); }
      ...
      // graceful placeholder if API fails
      const fallback = {
        ok:true,
